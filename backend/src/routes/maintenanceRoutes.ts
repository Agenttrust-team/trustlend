...
router.get('/maintenance', asyncHandler(async (req, res) => {
  ...
  if (!await cacheService.setNotExists('trustlend:maintenance-lock', lockId, 290)) {
    res.status(202).json({ status: 'already-running' }); return;
  }
  try {
    const contracts = [process.env.LOAN_MANAGER_CONTRACT_ID, process.env.LENDING_POOL_CONTRACT_ID, process.env.REMITTANCE_NFT_CONTRACT_ID, process.env.MULTISIG_GOVERNANCE_CONTRACT_ID].filter((id): id is string => Boolean(id));
    if (!contracts.length) throw AppError.serviceUnavailable('Contracts are not configured');
    const indexer = new EventIndexer({ rpcUrl: getStellarRpcUrl(), contractConfigs: contracts.map(contractId => ({ contractId })), batchSize: 1000, pollIntervalMs: 30000 });
